...
def downsample_flattened_lists(lst, downsample_factor, 
    ...
    # if lst[last_triplet_start:] not in downsampled_lst:   
    #     downsampled_lst.extend(lst[last_triplet_start:])
    # return downsampled_lst
...
def get_edge_similarity_convex(swc_file_1, swc_file_2, nrn1_matched_nodes, nrn2_matched_nodes, orientation=0.0, partition_length=1/2000):
    ...
    # ##
    # print('\nREPORTING UTILS')
    # tree1_paths = get_tree_paths(tree1, partition_length=1/20, downsampling_factor=1) #TODO just increase partition length to downscale branches and make this comp not seg fault???
    # tree2_paths = get_tree_paths(tree2, partition_length=1/20, downsampling_factor=1)
    # ##
