...
def _get_segment_path(critical_nodes, new_areas, partition_length, downsampling_factor):
    ...
    for kk in range(critical_nodes.shape[0] - 1):
        ...
        orientations = np.hstack(
            [
                orientations,
                vec_norm * np.kron(np.arange(1, this_edge_count), unit_vec.reshape(3, 1)),
                vec.reshape(3, 1),
            ]
        )
    ...
    for node in raw_morphology.nodes():
        if node in irreducible_nodes:
            ...
            for ancestor in node_to_root[1:]:
                ...
                if ancestor in irreducible_nodes:
                    break
            ...
            path_area = np.ones(
                num_edges
            )  # TODO add option to model edges as cylindars using node radii

            critical_nodes = pd.DataFrame(raw_path_to_irreducible_parent)[
                ["x", "y", "z"]
            ].to_numpy()

            lengths, orientations, areas, pillars = _get_segment_path(
                critical_nodes, path_area, partition_length, downsampling_factor
            )
...
def edges_between(descendent, ancestor, tree, tree_paths):
    ...
    while descendent != ancestor:
        if len(edge_lengths) > 0:
            pillars = np.hstack(
                (
                    pillars,
                    tree_paths[descendent["id"]]["pillars"][1:].astype(int) + len(edge_lengths),
                )
            )
            edge_orientations_x = np.append(
                edge_orientations_x, tree_paths[descendent["id"]]["orientations"][0, 1:]
            )
            edge_orientations_y = np.append(
                edge_orientations_y, tree_paths[descendent["id"]]["orientations"][1, 1:]
            )
            edge_orientations_z = np.append(
                edge_orientations_z, tree_paths[descendent["id"]]["orientations"][2, 1:]
            )
        else:
            pillars = tree_paths[descendent["id"]]["pillars"].astype(int)
            edge_orientations_x = np.append(
                edge_orientations_x, tree_paths[descendent["id"]]["orientations"][0]
            )
            edge_orientations_y = np.append(
                edge_orientations_y, tree_paths[descendent["id"]]["orientations"][1]
            )
            edge_orientations_z = np.append(
                edge_orientations_z, tree_paths[descendent["id"]]["orientations"][2]
            )
    ...
    edge_orientations = [
        coordinate for point in zip(*edge_orientations) for coordinate in point
    ]  # flatten orientations as: [x1,y1,z1,x2,y2,z2,x3,y3,z3,...,xn,yn,zn] -- this is important for quantized convex matching, but we could do this ouside edges_between fn.
